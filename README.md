...
* **No predefined group labels required.** ROME learns latent subgroup structure directly from data without requiring a predefined demographic partition.
* **Flexible use of sensitive attributes.** Sensitive features **S** can inform latent subgroup discovery through gating or membership models, while their direct use in group-specific outcome models can be restricted according to domain, institutional, policy, or legal requirements.
* **Two complementary formulations:**
  - **ROME-EM** — Expectation-Maximization with robust aggregation for linear models, connecting latent mixture components with maximin aggregation.
...
* **Competitive performance.** Evaluated against baselines spanning standard prediction, fair regression (BGL), adversarial reweighting (ARL), and mixture-of-experts across three real-world regression datasets.
...
@article{li2025rome,
  ...
  author={Li, Siqi and Liu, Molei and Lyu, Yiwei and Tian, Ziye and Hong, Chuan and Liu, Nan},
